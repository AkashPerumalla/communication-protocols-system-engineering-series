...
public class TokenMasterB {
    private static final String ARBITER_HOST = "localhost";
    private static final int ARBITER_PORT = 8020;
    private static final Logger LOGGER = Logger.getLogger(TokenMasterB.class.getName());
        ...
        try (Socket s = new Socket(ARBITER_HOST, ARBITER_PORT);
            ...
            while ((line = in.readLine()) != null) {
                if (line.equals("TOKEN")) {
                    ...
                    LOGGER.info("[B] Got TOKEN at " + ts + ", polling device on port " + port);
                    // Poll device
                    try (Socket ds = new Socket("localhost", port);
                        ...
                        LOGGER.info("[B] Device resp: " + resp);
                    } catch (IOException e) {
                        LOGGER.warning("[B] Poll failed: " + e.getMessage());
                    }
                    out.println("DONE_B");
                    deviceIdx++;
                }
